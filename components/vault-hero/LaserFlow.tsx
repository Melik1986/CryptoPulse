...
export function LaserFlow({
  ...
  const uniformsForMaterial = useMemo(() => {
    return {
      iTime: { value: 0 },
  ...
  useEffect(() => {
    ...
    uniformsRef.current.iResolution.value.set(size.width * dpr, size.height * dpr, dpr);
  }, [size]);
  ...
  useFrame((state, delta) => {
    ...
    if (!hasFadedRef.current) {
      const fadeDur = 1.0;
      const currentFade = uniforms.uFade.value as number;
      const newFade = Math.min(1, currentFade + cdt / fadeDur);
      uniforms.uFade.value = newFade;
      if (newFade >= 1) hasFadedRef.current = true;
    }
    ...
    const x = (mouse.x + 1) * 0.5 * widthPx;
    const y = heightPx - (mouse.y + 1) * 0.5 * heightPx;
  ...
  const planeSize = useMemo<[number, number]>(() => {
    return [viewport.width * 3, viewport.height * 3];
  }, [viewport]);
